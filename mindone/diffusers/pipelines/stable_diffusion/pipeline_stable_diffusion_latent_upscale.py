...
class StableDiffusionLatentUpscalePipeline(DiffusionPipeline, StableDiffusionMixin, FromSingleFileMixin):
    ...
    def _encode_prompt(self, prompt, do_classifier_free_guidance, negative_prompt):
        ...
        if do_classifier_free_guidance:
            ...
            text_pooler_out = mint.cat([uncond_pooler_out, text_pooler_out])
    ...
    def __call__(
        ...
        noise_level_embed = mint.cat(
            [
                mint.ones((text_pooler_out.shape[0], 64), dtype=text_pooler_out.dtype),
                mint.zeros((text_pooler_out.shape[0], 64), dtype=text_pooler_out.dtype),
            ],
            dim=1,
        )

        timestep_condition = mint.cat([noise_level_embed, text_pooler_out], dim=1)
