...
class SanaPAGPipelineFastTests(PipelineTesterMixin, unittest.TestCase):
    pipeline_config = [
        ...
        [
            ...
            "mindone.transformers.models.gemma.tokenization_gemma.GEMMATokenizer",
            dict(
    ...
    def get_dummy_components(self):
        ...
        gemma_config_cls = get_module("transformers.models.gemma2.configuration_gemma2.Gemma2Config")
        gemma_config = gemma_config_cls(
            head_dim=16,
            hidden_size=32,
        ...
        pt_text_encoder_cls = get_module("transformers.models.gemma2.modeling_gemma2.Gemma2Model")
        pt_text_encoder = pt_text_encoder_cls(gemma_config)
        ms_text_encoder_cls = get_module("mindone.transformers.models.gemma2.modeling_gemma2.Gemma2Model")
        ms_text_encoder = ms_text_encoder_cls(gemma_config)
    ...
    def test_inference(self, mode, dtype):
        ...
        torch.manual_seed(0)
        pt_image = pt_pipe(**inputs)[0][0]
        torch.manual_seed(0)
        ms_image = ms_pipe(**inputs)[0][0]
...
@slow
@ddt
class SanaPAGPipelineTests(PipelineTesterMixin, unittest.TestCase):
    @data(*test_cases)
    @unpack
    def test_inference(self, mode, dtype):
        ms.set_context(mode=mode)
