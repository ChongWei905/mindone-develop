...
class Attention(nn.Cell):
    ...
    def prepare_attention_mask(
        ...
        if current_length != target_length:
            ...
            attention_mask = F.pad(attention_mask, pad=((0, 0),) * (attention_mask.ndim - 1) + ((0, target_length),))
...
class XFormersAttnProcessor:
    ...
    def __call__(
        ...
        if attn.head_dim_padding > 0:
            query_padded = F.pad(query, (0, attn.head_dim_padding), mode="constant", value=0.0)
            key_padded = F.pad(key, (0, attn.head_dim_padding), mode="constant", value=0.0)
            value_padded = F.pad(value, (0, attn.head_dim_padding), mode="constant", value=0.0)
        else:
